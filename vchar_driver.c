...
#include <linux/cdev.h> /* thu vien any chua cac ham lam viec voi cdev */
#include <linux/uaccess.h>

#include "vchar_driver.h" // mo ta cac thanh ghi
...
int vchar_hw_read_data(vchar_dev_t *hw, int start_reg, int num_regs, char* kbuf)
{
    ...
    //tra ve so bytes da duoc doc tu cac thanh ghi du lieu
    return read_bytes;
}
...
static ssize_t vchar_driver_read(struct file *filp, char __user *user_buf, size_t len, loff_t *off)
{
    ...
    num_bytes = vchar_hw_read_data(vchar_drv.vchar_hw *off, kernel_buf);
